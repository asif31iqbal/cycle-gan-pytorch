...
the datasets from the original authors' source data \cite{data1}. The \textit{summer2winter} dataset contains 1 train and 1 test images for summer and 1 train and 1 test images for winter. The \textit{apple2orange} dataset contains 1 train and 1 test images for apple and 1 train and 1 test images for orange.

For the font style transfer, we have written python programs (included in the github repository) to generate images of single uppercase English characters and lowercase words. We obtain the words vocabulary from \cite{data2}. For the single uppercase character scenario, we generated 11 train images 10 test images for each character with random scaling and translation, so in total we got 286 train images and 260 test images. For the word scenario, we randomly sampled 1 words from \cite{data2} to create two disjoint datasets for training and testing, with 1 words for training and 1 words for testing. We also applied random scaling and translation to the words before generating the words. 

\section{Network Architecture}
We essentially rebuilt the same network architecture as the original authors \cite{cyclegan} have used.
We will go over the architecture briefly here.
...
\begin{figure*}[!htb]
     ...
     \begin{subfigure}[b]{\textwidth}
         ...
         \caption{Generator (number of channels at each stage shown in red)}
         \label{fig:gen}
     \end{subfigure}
     \vfill
     \begin{subfigure}[b]{0.6\textwidth}
         ...
         \caption{Resnet Block in Detail (number of channels at each stage shown in red)}
         \label{fig:res}
     \end{subfigure}
     \vfill
     \begin{subfigure}[b]{0.8\textwidth}
         ...
         \caption{Discriminator  (number of channels at each stage shown in red)}
         \label{fig:disc}
     \end{subfigure}
...
The generator has 3 main segments - Encoding, Transformation and Decoding. Figure 
\ref{fig:architecture}\subref{fig:gen} The encoding part has 3 general convolutional
layers each of which is a convolution followed by instance normalization \cite{cyclegan} and Relu.
The first of these use a kernel size of 7 and the second and third a kernel size of 3. Reflection padding
has been used on the input image to reduce artifacts. The transformation part is a series of 9 residual 
blocks (shown in more detail in Figure \ref{fig:architecture}\subref{fig:res}). Finally, the decoding segment 
is a couple of general deconvolutional layer (kernel sizes 3 and 2 respectively) followed by a convolution-
only (kernel size 7). Each deconvolutional layer is a deconvolution followed by instance normalization and 
Relu. The numbers of channels at all intermediate stage have been shown in red in \ref{fig:architecture}\subref{fig:gen}.

\subsection{Discriminator}
For the discriminator, we use the same $70 \times 70$ PatchGAN concept used in \cite{cyclegan}.
It basically has 4 general convolutional layers each of which is a convolution followed by instance 
normalization and a leaky relu with relu factor 0.2. These 4 layers are then followed by one single 
convolution only layer. The discriminator architecture and the numbers of channels at all intermediate 
stage have been shown in red in \ref{fig:architecture}\subref{fig:disc}.

Notice that both the generator and the discriminator networks are end-to-end fully convolutional. Before 
feeding our images to these networks, we resize them to $256 \times 256$. The output of the 
